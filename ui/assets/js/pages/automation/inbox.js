...
function row(letter) {
  ...
  return `<tr>
    ...
    <td>${d}</td>
    <td><span class="badge">${letter.status ?? 'جدید'}</span></td>
  </tr>`;
}

export default async function InboxPage(outlet) {
  const panel = document.createElement('div');
  panel.className = 'panel';
  panel.innerHTML = `
    <div class="toolbar">
      <div class="right"><h2 style="margin:0">نامه‌های دریافتی</h2></div>
      <div class="left">
        <input id="q" class="input" placeholder="جستجو در موضوع/فرستنده" />
  ...
  async function load() {
    ...
    try {
      const data = await API.getLetters('inbox', { q: q.value });
